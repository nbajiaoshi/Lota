...
//不追赶会导致进攻失利
//解决塔下不追人问题
//assemblestrategy不会判断怪在不在
//strengthcaculate 在远离队友的时候判断不准确
//---------------------------------------逃跑，集合打野，判断剩余防御塔，调参数
#include "sdk.h"
#include<ctime>
#include<iostream>
...
int StrongPushingStimulate(PUnits& PushHero,int RLimit = 2){
    int Hp[5],TowerHp,damage[5],maxRainLevel[5] = {-1,-1,-1,-1,-1},Attack_damge[5] = {0};
    ...
    int dam = 0;
    for (int i  = 0;i < PushHero.size();i++)
        dam += damage[i] * int(MAX((Hp[i] / (15 * maxRainLevel[i] + 55 / PushHero.size() + Attack_damge[i]/2) + 1)/2,RLimit));
    return dam;
}
//-------------------------------------------------------------------------------From Monster.cc
#define Maxid 100
...
//---------------------------------------------------------------------------------------------------
const PUnit* belongs(int id, PUnits& units) //???unit?????vector?а??????id??unit
{
...
class PullMonsterStrategy_1:public PullMonsterStrategy{
public:
    PullMonsterStrategy_1(string name = "PullMonsterStrategy_1"):PullMonsterStrategy(1,name){
        Max = 1;
...
class PullMonsterStrategy_0_0:public PullMonsterStrategy{
public:
    PullMonsterStrategy_0_0(string name = "PullMonsterStrategy_0_0"):PullMonsterStrategy(0,name){
        Max = 1;
        id[0] = 26 + 3 * INFO->camp;
...
class PullMonsterStrategy_0_1:public PullMonsterStrategy{
public:
    PullMonsterStrategy_0_1(string name = "PullMonsterStrategy_0_1"):PullMonsterStrategy(0,name){
        Max = 1;
        id[0] = 30 + 3 - 2 * INFO->camp;
        id[1] = 16 + 6 - 4 * INFO->camp;
        TargetSize = 2;
        TaskPos.clear();
        for (int i = 0;i < TargetSize;i++)
...
class FightNormalMonsterStrategy_0:public FightMonsterStrategy{
	public:
        FightNormalMonsterStrategy_0(string name = "FightNormalMonsterStrategy_0"):FightMonsterStrategy(name){
            Max = 3;
            id[0] = 30 + 2 - 2 * INFO->camp;
...
class FightNormalMonsterStrategy_3:public FightMonsterStrategy{
    public:
        FightNormalMonsterStrategy_3(string name = "FightNormalMonsterStrategy_3"):FightMonsterStrategy(name){
            Max = 3;
...
class FightNormalMonsterStrategy_1:public FightMonsterStrategy{
	public:
        FightNormalMonsterStrategy_1(string name = "FightNormalMonsterStrategy_1"):FightMonsterStrategy(name){
            Max = 1;
...
class FightNormalMonsterStrategy_2:public FightMonsterStrategy{
    public:
        FightNormalMonsterStrategy_2(string name = "FightNormalMonsterStrategy_2"):FightMonsterStrategy(name){
            Max = 1;
    ...
    for (int i = 0; i < num;i++){
        ...
        if (mind > 400)
            ...
            if (maxj == 4 && StrengthRate > 2 && enemyescape){
                ...
                fout << "RunAfter" << myhero->id << ' ' << RunAfterEnemyRounds[myhero->id % 5] << endl;
...
void Strategy::fightingMonster(const PUnit* a, const PUnit* b, PCommand &cmd)
{
    ...
    if (b->typeId == 6 || b->typeId == 7 || (b->typeId == 8 || b->typeId == 3) /*|| (dis2(a->pos,b->pos) <= a->range && a->findSkill("Attack")->cd == 0)*/) useSkill.clear();
...
void AssembleStrategy::FindSubTask(int id){
    ...
    if (!myhero->findBuff("Reviving")){
        ...
        for (int j = 1; j < SubTaskNum;j++) if (SubTask[j] && SubTask[j]->num < SubTask[j]->Max){
...
void AssembleStrategy::command(PCommand &cmd){
    ...
    for (int i = 0;i < SubTaskNum;i++)
        if (SubTask[i] && SubTask[i]->num > 0){
            ...
            SubTask[i]->clear();
            fout <<"SubTask  "<< SubTask[i]->TaskName << ":" << SubTask[i]->num << endl;
        }
    for (int i = 0; i < num;i++)
...
void FightDragonMonsterStrategy::command(PCommand &cmd){
    ...
    if (now >= TargetSize){
        int id = Taskid();
        Task[id] = new FightNormalMonsterStrategy_3();
        //Task[TotalTask++] = new PullMonsterStrategy_0_0();
        //Task[TotalTask++] = new PullMonsterStrategy_0_1();
        //Task[TotalTask++] = new PullMonsterStrategy_1();
        //if (num) Task[TotalTask - 1]->append(INFO->findUnitById(_hero[0]));
        for (int i = 0; i < 3 && i < num;i++){
            Task[id]->append(INFO->findUnitById(_hero[i]));
        }
        /*for (int i = 2; i < num;i++){
            cout << i << endl;
            Task[TotalTask - 4 + i]->append(INFO->findUnitById(_hero[i]));
        }
        cout << "??/n";*/
        Task[id]->command(cmd);
        return;
...
bool FightMonsterStrategy::TargetCanNotAttack(){
    ...
    if (reviving[id[now]] > 3 + minstep)
        return true;
    /*if (INFO->findUnitById(id[now])){
        reviving[id[now]] = monsters[INFO->findUnitById(id[now])->typeId - 6].reviving_time;
        fout << TaskName << " Monster " << id[now] <<' ' << reviving[id[now]] << endl;
    }*/
    for (int i = 0; i < 2;i++)
    ...
    if (t && t->max_hp * StrengthCaculate(t->pos,901,INFO->camp) < t->hp * unitEvaluation(t->name))
        return true;
    /*if (!t && StrengthCaculate(MonsterBornPos[id[now]],901,INFO->camp) < unitEvaluation(MonsterNameById(id[now])))
        return true;*/
    return false;
}
...
void FightNormalMonsterStrategy_3::command(PCommand &cmd){
    for (int i = 0; i < num;i++)
        if (dis2(INFO->findUnitById(_hero[i])->pos,SpringPos) > 2500){
            FightMonsterStrategy::command(cmd);
            return;
        }
    ChangeTask(new FightNormalMonsterStrategy_0,cmd);

}
void PullMonsterStrategy_0_0::command(PCommand &cmd){
    if (INFO->findUnitById(_hero[0]) && INFO->findUnitById(_hero[0])->level > 3)
        ChangeTask(new FightNormalMonsterStrategy_1,cmd);
    else
        PullMonsterStrategy::command(cmd);
}
void FightNormalMonsterStrategy_1::command(PCommand &cmd){
    if (INFO->findUnitById(_hero[0]) && INFO->findUnitById(_hero[0])->level < 4)
        ChangeTask(new PullMonsterStrategy_0_0,cmd);
    else
        FightMonsterStrategy::command(cmd);
}

void PullMonsterStrategy_1::command(PCommand &cmd){
    for (int i = 0;i < 5;i++)
        if (reviving[id[0]] > 10 && reviving[id[1]] > 10 && HeroTask[OurHero[i]->id] && HeroTask[OurHero[i]->id]->TaskName == "FightNormalMonsterStrategy_0"){
            TargetSize = 3;
        }
    PullMonsterStrategy::command(cmd);
    if (now < 2)
...
void PullMonsterStrategy::command(PCommand &cmd){
    ...
    for (int i = 0; i < num;i++){        
        myhero = INFO->findUnitById(_hero[i]);
        fout << "PullBow" << ' ' << myhero->id << ' ' << PullBowSign;
        if (!INFO->findUnitById(id[now])){
            if ((*myhero)["Exp"]->val[3])
                levelUp(myhero, cmd);
            else if (!PullBowSign)
            ...
            if (id[now] == 26 || id[now] == 29){
                ...
                if (dis2(myhero->pos,Monster->pos) <= Monster->range)
                    ...
                    fout << "PullMonster" << Monster->id << estimate->gettarget() << Monster->pos;
            ...
            if (dis2(FriendlyTower[TowerNum],Monster->pos) <= 100 && Monster->hp <= Tower_atk + myhero->atk - Monster->def * 2){
                ...
                fout << id[now] << ' ' << monsters[Monster->typeId - 6].reviving_time << endl;
            }
        }
    }
}
...
void prepared(){
    ...
    if (INFO->round == 1){
        ...
        Task[2] = new PullMonsterStrategy_1;
        Task[3] = new PullMonsterStrategy_0_0;
