...
int FightStrategy::BestMove(Operation& op,const PUnit* myhero,TargetEstimate* estimate){
    op.id = myhero->id;
    op.type = "Move";
    return TargetBFS(new FightingMoveEvaluation(myhero),myhero,1,op.targets,blocks_without_Tower_Rainy)->getvalue();
}
void FightStrategy::Stimulate(Operation& op){
    if (op.type == "Move")
        OurHeroPos[op.id % 5] = op.targets[op.targets.size() - 1];
    if (op.type == "Attack"){
        const PUnit* enemy = INFO->findUnitByPos(op.targets[op.targets.size() - 1]);
        ...
        if (mind > 400)
            ...
            case 1: //archer
                ...
                if (StrengthRate <= 2 && StrengthRate >= 0.5)
                    value[3] = BestMove(op[3],myhero,new FightingMoveEvaluation(myhero));
                if (StrengthRate > 2)
                    value[3] = BestMove(op[3],myhero,new RunAfterWeakEnemy(myhero));
                if (StrengthRate < 0.5)
                    value[3] = BestMove(op[3],myhero,new RunAwayFromPowerfulEnemy(myhero));
...
void Strategy::Move(const PUnit* myhero,PCommand &cmd,Pos target){
    ...
    if (OurHeroIntheRain[myhero->id % 5]){
        ...
        if (TowerBLock)
            findShortestPath(*MAP, myhero->pos, target, blocks, op.targets);
        else
            findShortestPath(*MAP, myhero->pos, target, blocks_without_Tower, op.targets);
    }
    if (op.targets[0] == Pos(-1,-1))
        findShortestPath(*MAP, myhero->pos, target, basic_blocks, op.targets);
    cmd.cmds.push_back(op);
}
    ...
    if (useSkill.size() && (b->isHero() || (a->mp > 150))) //策略：优先使用技能
    {
        ...
        if (dis2(a->pos, b->pos) <= a->range) //判断单位b是否在攻击范围内
        {
            if (a->findSkill("Attack")->cd == 1 && b->range < 64 && (b->atk - a->def) * 30 > a->max_hp){
                int max = 0;
                for (int i = 0; i < OurHero.size();i++)
                    if (dis2(a->pos,MonsterBornPos[b->id]) <= 324  && ang[b->id][OurHero[i]->id] > max) max = ang[b->id][OurHero[i]->id];
...
void PushTower::command(PCommand &cmd){
    ...
    //------
    for (int i = 0;i < EnemyHero.size();i++)
        if (dis2(EnemyHero[i]->pos,EnemyTower[TowerNum]) <= 625 && INFO->findUnitByPos(EnemyTower[TowerNum])->hp > 200){
            for (int j = 0;j < num;j++){
