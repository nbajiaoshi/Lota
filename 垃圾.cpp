...
int PushingTower = -1;
int unitEvaluation(string name) //?????????¦Ë???????
{

    if (name == string("Fighter") || name == string("Assassin") || name == string("Archer")) //???????§Ù???¦Ë???
        return -1;
    if (PushingTower > 0){
...
void levelUp(const PUnit* a, PCommand &cmd) //??¦Ëa????????
{
    Operation op;
    op.id = a->id;
...
void Strategy::fightingMonster(const PUnit* a, const PUnit* b, PCommand &cmd) //??¦Ëa???¦Ëb????????
{
    if (b->id >= 16 && b->id <= 36){
    ...
    if (useSkill.size() && (b->isHero() || (a->mp > 150))) //???????????¨¹???
    {
        ...
        if (dis2(a->pos, b->pos) <= a->range) //?§Ø??¦Ëb??????????¦¶??
        {
...
bool Strategy::FightMonster(PCommand &cmd,const PUnit* ptr){
    ...
    if (nearBy.size()) //??????????????§³?????????§Û???
    {
        const PUnit* ptr_foe = fixId[ptr->id]!=EMPTYID ?
                    belongs(fixId[ptr->id], nearBy) : NULL; //?§Ø???????????????
        if (!ptr_foe)
        {
            ptr_foe = nearBy[rand()%nearBy.size()];
            fixId[ptr->id] = ptr_foe->id; //????¦Ä???????????????????
        }
        fightingMonster(ptr, ptr_foe, cmd); //?????????
    } else
    {
        //findFoes(ptr, cmd); //????????§Ö§à???????????????
        return false;
    }
    return true;
}
