...
class TowerDefenceStrategy:public FightMonsterStrategy{
    public:
        TowerDefenceStrategy(string name = "TowerDefenceStrategy");
        virtual void NextTarget();
        virtual void command(PCommand &cmd);
};
class PushTower:public Strategy{
	protected:
		int TowerNum;
	public:
        PushTower(int n,string name = "PushTower"):Strategy(name),TowerNum(n){
            Max = 5;
            if (EnemyHeroTypeNum[2] >= 3 && (OurTowerDes[1] || OurTowerDes[0]))// assassin
                Max = 3;
...
void TowerDefenceStrategy::NextTarget(){
    now++;
    if (now >= TargetSize) now = now % TargetSize;
}
...
void AssembleStrategy::command(PCommand &cmd){
    ...
    {
        while (num > 3){
            myhero = INFO->findUnitById(_hero[0]);
            erase(myhero);
            int DefenceTaskid = -1;
...
void TowerDefenceStrategy::command(PCommand &cmd){
    if (OurTowerDes[1] && id[0] > 24){
        id[0] = 16 + 6 - 4 * INFO->camp;
        id[1] = 24 + 2 + 3 * INFO->camp;
        id[2] = 30 + 3 - 2 * INFO->camp;
...
TowerDefenceStrategy::TowerDefenceStrategy(string name):FightMonsterStrategy(name){
    Max = 2;
    if (OurTowerDes[1]){
        id[0] = 16 + 6 - 4 * INFO->camp;
        id[1] = 24 + 2 + 3 * INFO->camp;
