...
int FarAwayFromEnemy::evaluate(Pos pos){
    int valu = 0;
    for (int i = 0; i < EnemyHero.size();i++){
        valu -= 100*(dis2(EnemyHero[i]->pos,pos) <= 625?abs (15 - dis(EnemyHero[i]->pos,pos)):0);
    }
    return valu;
}
...
void AssembleStrategy::GetSubTask(){
    ...
    if (OurTowerDes[0] && !OurTowerDes[1]){
        if (SubTaskStatus != 1){
            ...
            SubTask[1] = new FightMonsterStrategy(2,32,21,20,0);
            SubTaskNum = 2;
            SubTaskStatus = 1;
        }
        return;
...
void AssembleStrategy::command(PCommand &cmd){
    ...
    for (int i = 0;i < num;i++)
        fout << _hero[i] <<' '<< HeroTask[_hero[i]]->TaskName << endl;
...
void FightDragonMonsterStrategy::command(PCommand &cmd){
    ...
    if (now >= TargetSize){
        ...
        //Task[TotalTask++] = new PullMonsterStrategy_26_22();
        //Task[TotalTask++] = new PullMonsterStrategy_0_1();
        //Task[TotalTask++] = new PullMonsterStrategy_DogAndWolf();
...
bool FightMonsterStrategy::TargetCanNotAttack(){
    ...
    if (!t){
        ...
        int totalLevel = 0;
        for (int i = 0;i < num;i++){
            fout << dis(INFO->findUnitById(_hero[i])->pos,MonsterBornPos[id[now]])  << endl;
            if (dis(INFO->findUnitById(_hero[i])->pos,MonsterBornPos[id[now]]) < mindis + 13)
                totalLevel += (3 + INFO->findUnitById(_hero[i])->level);
        }
        fout << "monster" << totalLevel << ' ' << unitEvaluation(MonsterNameById(id[now])) << endl;
...
void prepared(){
    ...
    if (TowerAttackValue){
        ...
        for (int i = 0; i < EnemyHero.size();i++)
            for (int j = 0;j < 1;j++)
                if (!towerDes[j] && dis2(EnemyHero[i]->pos,EnemyTower[j]) > 9 && dis2(EnemyHero[i]->pos,EnemyTower[j]) < 900)
